...
class wfm {
    ...
    public function __construct($connection_options=null) {
        ...
        if (count($this->settings)==0) {
            ...
            $sql->exec("CREATE TABLE `".$dbtable_prefix."WFM` (
                                     `".$dbtable_prefix."WFM_id` int NOT NULL AUTO_INCREMENT PRIMARY KEY,
                                     `".$dbtable_prefix."WFM_IP` varchar(45) NOT NULL DEFAULT '',
                                     `".$dbtable_prefix."WFM_uri` varchar(256) NOT NULL DEFAULT '',
            ...
            $sql->exec("CREATE TABLE `".$dbtable_prefix."WFM_Log` (
                                     ...
                                     `".$dbtable_prefix."WFM_Log_address` varchar(50) NOT NULL DEFAULT '',
                                     `".$dbtable_prefix."WFM_Log_address_ref` varchar(50) NOT NULL DEFAULT '',
            ...
            $sql->exec("CREATE TABLE `".$dbtable_prefix."WFM_vLog` (
...
<?php
    ...
    public function is_valid(&$return_exact_error) {
        global $antibotlinks, $data, $sql, $dbtable_prefix;
...
<?php
    ...
    public function admin_config_top() {
        global $sql, $currency, $session_prefix, $dbtable_prefix;
...
function wfm_claim_loop() {
    $.post(\''.basename($_SERVER['PHP_SELF']).'\', {action:\'ajax_wfm_get_claim_log\', last_id:wfm_claim_last_id, csrftoken:\''.$_SESSION[$session_prefix.'_csrftoken'].'\'})
        .done(function(jsonData) {
            if (jsonData!=\'\') {
                ...
                $(\'.c_report\').click(function(){
                    ...
                    $.post(\''.basename($_SERVER['PHP_SELF']).'\', {action:\'ajax_wfm\', rid:$(this).attr(\'rel\'), msg:report_msg, csrftoken:\''.$_SESSION[$session_prefix.'_csrftoken'].'\'})
                        .done(function(data) {
                            if (data!=\'\') {
                                alert(data);
                            }
...
function wfm_visit_loop() {
    $.post(\''.basename($_SERVER['PHP_SELF']).'\', {action:\'ajax_wfm_get_visit_log\', last_id:wfm_visit_last_id, csrftoken:\''.$_SESSION[$session_prefix.'_csrftoken'].'\'})
        .done(function(jsonData) {
            if (jsonData!=\'\') {
                var data=JSON.parse(jsonData);
