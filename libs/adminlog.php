...
class adminlog {
    ...
    public function __construct($connection_options=null) {
        global $sql, $faucet_settings_array, $session_prefix, $dbtable_prefix;
        ...
        //
        if (!empty($_POST)) {
            if ((!empty($_SESSION[$session_prefix.'_address_input_name']))&&(!empty($_POST[$_SESSION[$session_prefix.'_address_input_name']]))) {
                ...
                $q->execute(array(time(), session_id().'-'.getUniqueRequestID(), $_POST[$_SESSION[$session_prefix.'_address_input_name']]));
            ...
            if (!empty($_POST['action'])) {
                switch ($_POST['action']) {
                    case 'ajax_admin_get_log':
                        ...
                        $adminlog_query_raw_order="ORDER BY ".$dbtable_prefix."Sessions_Log_time DESC LIMIT 2500;";
    ...
    public function admin_config() {
        global $sql, $page, $currency, $session_prefix, $dbtable_prefix;
...
function adminlog_claim_loop() {
    $.post(\''.basename($_SERVER['PHP_SELF']).'\', {action:\'ajax_admin_get_log\', last_id:adminlog_claim_last_id, csrftoken:\''.$_SESSION[$session_prefix.'_csrftoken'].'\'})
